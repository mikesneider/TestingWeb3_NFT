...
contract PlatziPunksDNA {
        ...
        returns (uint256)
    {
        uint256 combineParams =  _tokenId + uint160(_minter);
         //adress es una dirección hexadecimal, entonces puedo somularlos a un entero pasandolo a entero
        bytes memory ecodedParams = abi.encodePacked(combineParams);
        bytes32 hashedParams = keccak256(ecodedParams);
        //esto lo hace pseudo aleatorio pero podría hacerlo con chain.link
